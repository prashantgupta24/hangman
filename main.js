$(function() {
  ...
  hangman.chancesElem = $('#chances');
  hangman.incorrectElem = $('#incorrect');
  hangman.noteElem = $('#note');

  hangman.initialize();

  $(document).keypress(function (event) {
    if(hangman.playing) {
      $(hangman.noteElem).text('');
      let keyCode = event.which || event.keyCode;
      //console.log(keyCode);
      if((keyCode >= 65 && keyCode<=90) || (keyCode>=97 && keyCode<=122)) {
        hangman.processKey(keyCode);
      } else {
        $(hangman.noteElem).text('Please enter alphabets only!');
      }
    }
  });
